...
// 量级差一两个数量级），配不出对角 R。端点是检测器的直接输出，两个端点的
// 定位误差近似独立，R 自然按端点分块，见 lightCov。
//
// 不再先折成 [角度, 中心, 长度] 再观测：四个量都由同两个端点算出，彼此相关，
// 配对角 R 与端点噪声对不上；角度一维还得处理 ±π 缠绕。直接观测端点，残差
// 就是普通减法。
...
// 一根灯条的 4×4 观测协方差。
//
// 两个端点的定位误差互相独立，所以是两个 2×2 块、块间为零。每块在灯条自身
// 坐标系里是对角的——沿灯条 sigma_along、垂直灯条 sigma_perp——再旋到图像系：
