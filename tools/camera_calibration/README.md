...
xmake run camera_capture -- config/camera_config.yaml \
...
如果只需要相机内参和畸变参数，棋盘格真实边长不会影响标定结果。程序默认使用毫米为单位的示例值：
...
先确认 [config/camera_config.yaml](../../config/camera_config.yaml) 中的相机配置正确，例如：
...
- `config/camera_config.yaml`：相机配置文件。
- `--output-dir`：照片保存根目录。
...
| `--cols` | `11` | 水平方向内角点数量 |
| `--rows` | `8` | 垂直方向内角点数量 |
| `--square-size` | `15.0` | 单格边长，内参标定可使用任意一致单位 |
| `--max-views` | `1000` | 最多使用的多样视角数量 |
| `--min-views` | `20` | 标定所需的最少有效视角 |
| `--max-sharpness` | `5.0` | 最大边缘过渡宽度，单位为像素 |
| `--min-contrast` | `30.0` | 黑白区域最小灰度差 |
| `--min-area` | `0.005` | 内角点凸包占图像面积的最小比例 |
| `--bootstrap` | `0` | 参数重采样次数，设为正数可启用 |
...
使用 OpenCV `findChessboardCornersSB` 检测 `11 × 8` 个内角点，并启用：
...
选择差异最大的视角，默认最多使用 1000 张。连续重复照片会被降低优先级。
...
[config/camera_config.yaml](../../config/camera_config.yaml)
...
查看去畸变预览
    ↓
将 calibration 节点写入 camera_config.yaml
    ↓
