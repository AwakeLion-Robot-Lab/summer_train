...
方向"的角。这里参数交换，量的是**偏离竖直方向**的角。灯条基本竖直，于是灯条的物理倾斜
直接对应 α 的小变化，∂α/∂状态 的条件数比标准写法好。

**但 α 的绝对值不在 0 附近，而在 ±π 附近。**实测（`tests/uvl_measure_smoke.cpp` 用
1210 焦距、3 m 距离）：
...
下端点            v = 507.269
Δy = -23.61,  Δx = -0.87
α  = -3.1047 rad          π = 3.1416
```

原因是 `armorLightPoints3D` 的第一个点取世界 **+z（上）**，而相机光学系的 y 轴**朝下**，
所以投影后"上端点"的 v 反而更小，Δy < 0，`atan2(Δx, Δy)` 自然落到 ±π。awakening 的
3D 点序（`build_light` 先 +H/2）和检测角点序（先 `LEFT_TOP`）都是这个方向，预测与观测
同号，所以滤波器完全不受影响——残差走 `normalizeAngle` 就抹平了。

**这件事有个不该忽略的后果：角度观测常年贴着 ±π 的分支切口。**于是 6.2 节里"求 H 的
中心差分差的是 residual 而不是 z_pred"就**不是防御性写法，而是必需的**——扰动只要跨过
切口，直接差 z_pred 就会得到一个 2π 的假梯度，H 的那一列彻底作废。照搬时这一行不能省。
...
本仓库的 `ErrorStateEkf` 已经删掉了 `delta_x`：它只会被赋 0，而 F·0 仍是 0，所以就算
连续预测多次再更新，它也一直是 0。迭代直接从 `δ = 0` 出发，关联门限也在 `δ = 0` 处线性化。
...
而由 5.4 节的实测，α **常年就贴在 ±π 上**，所以这不是小概率的边界情况，是每一帧都在
发生的事。这一行不能省。
