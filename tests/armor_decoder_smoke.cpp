...
void checkYolov8Layout()
{
  ...
  const L2Perception::ArmorDecoder decoder(L2Perception::yolov8Preset());
...
int main()
{
  try {
    ...
    const auto rejects = [](const L2Perception::ArmorDecoder& checked,
                            const std::vector<L2Perception::InferenceOutputSpec>& outputs) {
      try {
        checked.validate(outputs);
      } catch (const std::runtime_error&) {
        return true;
      }
      return false;
    };
    const L2Perception::ArmorDecoder yolov5_decoder(L2Perception::yolov5Preset());
    const L2Perception::ArmorDecoder yolov8_decoder(L2Perception::yolov8Preset());
    require(!rejects(yolov5_decoder, {{"output", {1, 25200, 22}}}),
            "validate rejected a matching yolov5_22 output");
    require(!rejects(yolov8_decoder, {{"output0", {1, 21, 6300}}}),
            "validate rejected a matching yolov8_21 output");
    require(rejects(yolov5_decoder, {{"output0", {1, 21, 6300}}}),
            "validate accepted a yolov8 model under the yolov5_22 layout");
    require(rejects(yolov8_decoder, {{"output0", {1, 11, 8400}}}),
