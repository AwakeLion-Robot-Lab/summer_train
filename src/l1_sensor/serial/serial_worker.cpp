...
std::optional<L5Control::SerialCommand> SerialWorker::latestSentCommand() const
{
...
void SerialWorker::rxLoop()
{
  ...
  while (running_.load()) {
    ...
    for (const auto& state : states) {
      if (!gimbal_history_.empty() && state.timestamp <= gimbal_history_.back().timestamp) {
        L6Telemetry::logWarn("serial gimbal history timestamp is not increasing");
        continue;
      }
...
void SerialWorker::txLoop()
{
  ...
  const auto period = std::chrono::microseconds(
    std::max(1, 1'000'000 / tx_rate_hz));
  ...
  while (running_.load()) {
    ...
    bool command_expired = false;
    {
      ...
      if (command_is_fresh) {
        ...
        // 协议没有独立的 hold/valid 位；保留最后设定值以避免云台突跳，强制关火。
        if (has_command_) {
          command = latest_command_;
    ...
    if (command_expired) {
      L6Telemetry::logWarn(
        "serial command expired; sending safe command",
  ...
  std::chrono::steady_clock::time_point timestamp) const
{
  ...
  const auto to_quaternion = [](const RobotState& state) {
    return L6Telemetry::rpyToQuaternion(state.rpy.roll, state.rpy.pitch, state.rpy.yaw);
  };
  ...
  const auto upper = std::lower_bound(
    gimbal_history_.begin(),
    gimbal_history_.end(),
    timestamp,
    [](const RobotState& state, const std::chrono::steady_clock::time_point& t) {
  ...
  return L6Telemetry::slerpQuaternion(
