...
template <typename T>
T readOptional(const YAML::Node& node, const std::string& key, const T& default_value)
{
  if (!node[key]) {
...
SerialConfig loadSerialConfig(const std::string& config_path)
{
  ...
  config.read_timeout_ms = readOptional(yaml, "read_timeout_ms", config.read_timeout_ms);
  ...
  config.rx_buffer_size = readOptional(yaml, "rx_buffer_size", config.rx_buffer_size);
  config.packet_loss_check_enable =
    readOptional(yaml, "packet_loss_check_enable", config.packet_loss_check_enable);
