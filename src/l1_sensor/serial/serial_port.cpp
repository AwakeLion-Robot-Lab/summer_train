...
// 调用方持有 lifecycle_mutex_ 的独占锁；避免 close 与 read/write 并发访问 SDK 句柄。
...
bool SerialPort::isOpenLocked() const
{
  ...
  bool close_after_error = false;
  {
    ...
    try {
      ...
      if (bytes_written != data.size()) {
        L6Telemetry::logWarn("serial write incomplete", bytes_written, data.size());
      }
