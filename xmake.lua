...
target("newvision")
    ...
    add_includedirs("$(projectdir)/.deps/eigen-3.4.0", {public = true})
...
target("planner_tracking_smoke")
    set_kind("binary")
    set_default(false)
    set_rundir("$(projectdir)")
    add_files("tests/planner_tracking_smoke.cpp")
    add_deps("newvision")

target("tiny_mpc_smoke")
    set_kind("binary")
    set_default(false)
    set_rundir("$(projectdir)")
    add_files("tests/tiny_mpc_smoke.cpp")
    add_deps("newvision")

target("planner_config_smoke")
    set_kind("binary")
    set_default(false)
    set_rundir("$(projectdir)")
    add_files("tests/planner_config_smoke.cpp")
    add_deps("newvision")
